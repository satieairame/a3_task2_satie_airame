...
mpg_model_class <- mpg %>%
  unite(col = model_class, # Create a new column called 'model_class'
        ...
        sep = "-", # Puts "-" between the pieces (default is underscore),
        remove = FALSE) # Do not remote original columns.
```
...
some_dates <- c(05031970, 09021953, 10291997) # Creates a vector of dates w/o spaces
...
storms_danger <- storms %>% # Start from storms data
  mutate(danger = case_when( # Add a new column danger, which contains... 
    ...
    status = TRUE ~ "moderate", # "moderate" if status is tropical storm
  )
  )
